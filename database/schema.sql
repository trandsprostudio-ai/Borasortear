-- 1. Função de Sorteio Totalmente Explícita
...
BEGIN
  ...
  -- Busca dados da sala (Sem JOIN para evitar ambiguidade)
  SELECT module_id, max_participants, current_participants
  INTO v_module_id, v_max_p, v_total_participants
  FROM public.rooms
  WHERE id = p_room_id;
...
-- 2. Gatilho de Substituição de Sala Corrigido
CREATE OR REPLACE FUNCTION public.replace_finished_room()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER AS $function$
BEGIN
  IF NEW.status = 'finished' AND OLD.status != 'finished' THEN
    IF NOT EXISTS (SELECT 1 FROM public.rooms WHERE module_id = OLD.module_id AND status = 'open') THEN
      INSERT INTO public.rooms (module_id, max_participants, expires_at, status)
      VALUES (OLD.module_id, OLD.max_participants, NOW() + INTERVAL '3 hours', 'open');
    END IF;
  END IF;
  RETURN NEW;
END;
...
-- 3. Gatilho de Participante Corrigido (Removendo Ambiguidade no RETURNING)
...
DECLARE
  ...
  v_max int;
BEGIN
  -- Atualiza o contador da sala de forma explícita
  UPDATE public.rooms 
  SET current_participants = current_participants + 1 
  WHERE id = NEW.room_id
  RETURNING current_participants, max_participants INTO v_current, v_max;
