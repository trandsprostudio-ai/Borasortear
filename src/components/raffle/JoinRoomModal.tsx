      ...
      <DialogContent className="glass-card border-white/10 rounded-[2.5rem] max-w-sm">
        {isRestricted ? (
          <div className="text-center py-6">
            ...
            <Button onClick={onClose} className="w-full h-14 rounded-2xl bg-white/5 text-white font-black">ENTENDIDO</Button>
          </div>
        ) : !ticketCode ? (
          <>
            ...
            <DialogFooter className="flex flex-col gap-2 sm:flex-col">
              <Button onClick={handleJoin} disabled={isJoining || userBalance < module.price} className="w-full premium-gradient h-14 rounded-2xl font-black text-lg">
                {isJoining ? <Loader2 className="animate-spin" /> : 'CONFIRMAR E ENTRAR'}
              </Button>
              <Button variant="ghost" onClick={onClose} className="w-full h-12 text-white/20 font-black text-[10px] uppercase">Cancelar</Button>
            </DialogFooter>
          </>
        ) : (
