...
import { AlertTriangle, Trophy, Users, Info, Zap, Star, Wallet } from 'lucide-react';
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="glass-card border-white/10 rounded-[2.5rem] max-w-md p-0 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-[#111827] via-[#111827] to-transparent border-t border-white/5">
          <div className="grid grid-cols-2 gap-3">
            <Button variant="ghost" onClick={onClose} className="h-14 rounded-xl font-black text-xs uppercase border border-white/10 text-white hover:bg-white/5">VOLTAR</Button>
            <Button 
              onClick={handlePreConfirm}
              disabled={loading}
              className={`h-14 rounded-xl premium-gradient font-black text-xs uppercase text-white shadow-xl shadow-blue-600/20 ${!hasFunds && !loading ? 'opacity-80' : ''}`}
            >
              {loading ? (
