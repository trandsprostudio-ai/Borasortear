...
import { Plus, Zap, Trash2, RefreshCw, LayoutGrid } from 'lucide-react';
...
const AdminSystem = () => {
  ...
  const handleCreateRoom = async (moduleId: string, maxParticipants: number) => {
    const { error } = await supabase.from('rooms').insert({
      ...
      status: 'open',
      expires_at: new Date(Date.now() + 5 * 60 * 60 * 1000).toISOString() // 5 horas
    });
  ...
  return (
    <div className="space-y-10">
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-black italic tracking-tighter uppercase flex items-center gap-3">
            <LayoutGrid className="text-purple-500" /> Módulos de Sorteio
          </h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {modules.map((mod) => (
            <div key={mod.id} className="glass-card p-6 rounded-2xl border-white/5 text-center group">
              ...
              <Button 
                ...
                className="w-full h-10 bg-purple-600 hover:bg-purple-700 text-[10px] font-black uppercase tracking-widest rounded-xl"
              >
                <Plus size={14} className="mr-1" /> Criar Mesa
              </Button>
            </div>
          ))}
        </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-black italic tracking-tighter uppercase flex items-center gap-3">
            <Zap className="text-amber-500" /> Mesas em Tempo Real
          </h3>
